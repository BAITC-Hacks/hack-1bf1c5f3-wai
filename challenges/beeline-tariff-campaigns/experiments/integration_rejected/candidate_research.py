...
target-tariff hypotheses from a hierarchical historical model. We retain every
cell and use weak priors: pilots reveal the effect for the evaluation audience.
"""
...
def build_hypotheses(
    ...
    The model shares evidence across destinations and price gaps, including
    transitions missing from history. Historical sampling precision is not
    confidence about a different audience: all candidates retain SD 0.15.
    ...
    for cell in build_cells(profile):
        options = [
            (priors[(cell.current_tariff, target, cell.arpu_segment)], target)
            for target in targets
            if target != cell.current_tariff
            and (cell.current_tariff, target, cell.arpu_segment) in priors
        ]
        if not options:
